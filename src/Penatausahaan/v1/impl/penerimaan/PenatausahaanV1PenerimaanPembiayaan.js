...
export class PenatausahaanV1PenerimaanPembiayaan extends PenatausahaanAPI {

	async sipdSelectAll(idSubSKPD) {
		return new Promise((resolve, reject) => {
			let URL = `https://service.sipd.kemendagri.go.id/referensi/strict/dpa/penerimaan/pembiayaan/${idSubSKPD}`;
	...
	async espressoSelectAll() {
		return new Promise((resolve, reject) => {
			let URL = this.espressoHost() + "/sipd/v1/penatausahaan/penerimaan/pembiayaan";
	...
	async update(indexFrom = 0, indexInto = 0, delay = 0) {
		...
		for (let i = indexFrom; i < Math.min(indexInto, listDataEspresso.length); i++) {
			...
			try {
				...
				console.log(`await penatausahaanPenerimaanPembiayaan.update(${i}, ${indexInto}, ${delay});`)
				break;
			}
		}
	}
