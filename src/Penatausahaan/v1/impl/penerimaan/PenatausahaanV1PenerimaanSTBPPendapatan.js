...
export class PenatausahaanV1PenerimaanSTBPPendapatan extends PenatausahaanAPI {
	...
	async sipdSelectAll(idSKPD = 0, page = 1) {
		return new Promise((resolve, reject) => {
			...
			let pageMax = 0;
			let URL = `https://service.sipd.kemendagri.go.id/penerimaan/strict/stbp` + (params.length === 0 ? "" : ("?" + params.join("&")));
	...
	async sipdSelectById(idSTBP) {
		return new Promise((resolve, reject) => {
			let URL = `https://service.sipd.kemendagri.go.id/penerimaan/strict/stbp/cetak/${idSTBP}`;
	...
	async espressoSelectID() {
		return new Promise((resolve, reject) => {
			let URL = this.espressoHost() + "/sipd/v1/penatausahaan/penerimaan/stbp/pendapatan";
			fetch(URL, this.getRequest()).then(response => response.json()).then(JSON => resolve(JSON)).catch(error => reject(error));
		});
	}
	...
	async espressoUpdateRincian(idSTBP, body) {
		return new Promise((resolve, reject) => {
			let URL = this.espressoHost() + `/sipd/v1/penatausahaan/penerimaan/stbp/pendapatan/${idSTBP}`;
			fetch(URL, this.postRequest(body)).then(response => response.json()).then(JSON => resolve(JSON)).catch(error => reject(error));
		});
	}
		...
		for (let i = indexSKPD ; i < listSKPD.length ; i++) {
			...
			while (true) {
				try {
					...
					console.log(`await penatausahaanPenerimaanSTBPPendapatan.update(${indexSKPD}, ${pageFrom}, ${delay});`);
					break;
				}
			}
	...
	async updateRincian(indexFrom = 0, indexInto = 0, delay = 0) {
		...
		for (let i = indexFrom ; i < Math.min(indexInto, listID.length) ; i++) {
			...
			try {
				...
				console.log(`await penatausahaanPenerimaanSTBPPendapatan.updateRincian(${indexFrom}, ${indexInto}, ${delay});`);
				break;
			}
		}
	}
