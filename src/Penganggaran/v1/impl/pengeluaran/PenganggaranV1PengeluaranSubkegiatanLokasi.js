...
export class PenganggaranV1PengeluaranSubkegiatanLokasi extends PenganggaranV1API {
	...
	async sipdFindByIdPenganggaranSubkegiatan(idPenganggaranSubkegiatan = null, isAnggaran = 1) {
		return await new Promise(async (resolve, reject) => {
			...
			let url = `https://sipd-ri.kemendagri.go.id/api/renja/detil_lokasi_bl/get_by_id_sub_bl`;
	...
	async espressoSaveAll(body) {
		return await new Promise(async (resolve, reject) => {
			let url = `${this.espressoHost()}/sipd/v1/penganggaran/pengeluaran/subkegiatan_lokasi`;
