...
export class PenganggaranV1PengeluaranSubkegiatanTag extends PenganggaranV1API {

	constructor() {
		super();
		this._title = "data_penganggaran_pengeluaran_subkegiatan_tag";
	}
	...
	async sipdFindByIdPenganggaranSubkegiatan(idPenganggaranSubkegiatan = null, isAnggaran = 1) {
		return await new Promise(async (resolve, reject) => {
			...
			let url = `https://sipd-ri.kemendagri.go.id/api/renja/tag_bl/get_by_id_sub_bl`;
	...
	async espressoSaveAll(body) {
		return await new Promise(async (resolve, reject) => {
			let url = `${this.espressoHost()}/sipd/v1/penganggaran/pengeluaran/subkegiatan_tag`;
