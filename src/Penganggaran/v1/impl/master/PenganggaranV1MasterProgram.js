...
export class PenganggaranV1MasterProgram extends PenganggaranV1API {

	constructor() {
		super();
		this._title = "penganggaran_master_program";
	}

	async sipdFindAll() {
		return await new Promise(async (resolve, reject) => {
			let url = `https://sipd-ri.kemendagri.go.id/api/master/program/list`;
			...
			opt.body.set_prov = this.isProvinsi();
			opt.body.set_kab_kota = this.isKabupatenKota();
	...
	async espressoSaveAll(body) {
		return await new Promise(async (resolve, reject) => {
			let url = `${this.espressoHost()}/sipd/v1/penganggaran/master/program`;
