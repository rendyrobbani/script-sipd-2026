...
export class PenganggaranV1MasterLabelProvinsi extends PenganggaranV1API {

	constructor() {
		super();
		this._title = "penganggaran_master_label_provinsi";
	}

	async sipdFindAll() {
		return await new Promise(async (resolve, reject) => {
			let url = `https://sipd-ri.kemendagri.go.id/api/master/label_prov/list`;
			...
			opt.body.id_daerah = this.idProvinsi();
	...
	async espressoSaveAll(body) {
		return await new Promise(async (resolve, reject) => {
			let url = `${this.espressoHost()}/sipd/v1/penganggaran/master/label/provinsi`;
