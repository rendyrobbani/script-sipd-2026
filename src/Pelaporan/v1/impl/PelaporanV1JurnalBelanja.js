...
export class PelaporanV1JurnalBelanja extends PelaporanAPI {
	...
	async espressoUpdate(body) {
		return new Promise((resolve, reject) => {
			let URL = this.espressoHost() + "/sipd/v1/pelaporan/jurnal/belanja";
	...
	async update(pageFrom = 1, pageInto = 0, delay = 0) {
		while (true) {
			try {
				...
				console.log(`await pelaporanJurnalBelanja.update(${pageFrom}, ${pageInto}, ${delay});`);
				break;
			}
		}
	}
	...
	async updateByListNomor(listNomor = [], delay = 0) {
		for (let i = 0; i < listNomor.length; i++) {
			...
			try {
				...
				console.log(`await pelaporanJurnalBelanja.updateByListNomor(${JSON.stringify(listNomor.slice(i))}, ${delay});`);
				break;
			}
		}
	}

	async postingSP2D(id) {
		return new Promise((resolve, reject) => {
			let body = {};
			body.id = id;
			body.tipe = "sp2d";
			...
			let URL = `https://service.sipd.kemendagri.go.id/aklap/api/belanja/approve`;
			fetch(URL, this.postRequest(body)).then(response => response.json()).then(JSON => resolve(JSON)).catch(error => reject(error));
		});
	}

	async unpostingSP2D(id) {
		return new Promise((resolve, reject) => {
			let body = {};
			body.id = id;
			body.tipe = "sp2d";

			let URL = `https://service.sipd.kemendagri.go.id/aklap/api/belanja/unapprove`;
			fetch(URL, this.postRequest(body)).then(response => response.json()).then(JSON => resolve(JSON)).catch(error => reject(error));
		});
	}
