...
export class PelaporanV1JurnalKoreksi extends PelaporanAPI {
	...
	async espressoUpdate(body) {
		return new Promise((resolve, reject) => {
			let URL = this.espressoHost() + "/sipd/v1/pelaporan/jurnal/koreksi";
	...
	async update(pageFrom = 1, pageInto = 0, delay = 0) {
		while (true) {
			try {
				...
				console.log(`await pelaporanJurnalKoreksi.update(${pageFrom}, ${pageInto}, ${delay});`);
				break;
			}
		}
	}
	...
	async updateByListNomor(listNomor = [], delay = 0) {
		for (let i = 0; i < listNomor.length; i++) {
			...
			try {
				...
				console.log(`await pelaporanJurnalKoreksi.updateByListNomor(${JSON.stringify(listNomor.slice(i))}, ${delay});`);
				break;
			}
		}
	}
